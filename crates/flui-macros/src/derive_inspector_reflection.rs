...
fn generate_reflected_trait(trait_item: ItemTrait) -> TokenStream {
    ...
    // Determine if we're being called from within the flui-core crate
    let call_site = Span::call_site();
    let inspector_reflection_path = if is_called_from_flui_core_crate(call_site) {
        quote! { crate::inspector_reflection }
    } else {
        quote! { ::flui_core::inspector_reflection }
    };
...
fn is_called_from_flui_core_crate(_span: Span) -> bool {
    // Check if we're being called from within the flui-core crate
    std::env::var("CARGO_PKG_NAME").is_ok_and(|name| name == "flui-core")
}
...
fn try_expand_macro(macro_item: &syn::TraitItemMacro) -> Option<Vec<TraitItem>> {
    ...
    match macro_name.as_str() {
        "flui_macros::style_helpers" | "style_helpers" => {
