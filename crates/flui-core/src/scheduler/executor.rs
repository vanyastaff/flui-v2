...
/// An executor for running tasks on the foreground (main/UI) thread.
#[derive(Clone)]
...
impl ForegroundExecutor {
    /// Creates a new ForegroundExecutor for the given session.
    ...
    /// Returns the session ID associated with this executor.
    ...
    /// Spawns a task to run on the foreground thread.
    #[track_caller]
    ...
    /// Creates a timer that resolves after the given duration.
    #[track_caller]
    pub fn timer(&self, duration: Duration) -> Timer {
        self.scheduler.timer(duration)
    }

    /// Returns the current monotonic instant from the scheduler's clock.
...
impl BackgroundExecutor {
    /// Creates a new BackgroundExecutor.
    pub fn new(scheduler: Arc<dyn Scheduler>) -> Self {
        Self { scheduler }
    }

    /// Spawns a background task with default priority.
    #[track_caller]
    pub fn spawn<F>(&self, future: F) -> Task<F::Output>
    where
    ...
    /// Spawns a background task with the given priority.
    #[track_caller]
...
impl<T> Task<T> {
    ...
    /// Returns true if the task has completed and its value is ready.
