...
/// Interface for scheduling tasks on different threads or execution contexts.
pub trait Scheduler: Send + Sync {
    ...
    /// Schedules a task to run on the foreground (main/UI) thread.
...
/// A unique identifier for a window or application session.
#[derive(Copy, Clone, Debug, Eq, PartialEq, Ord, PartialOrd, Hash)]
pub struct SessionId(u16);

impl SessionId {
    /// Creates a new SessionId from the given u16.
    pub fn new(id: u16) -> Self {
        SessionId(id)
    }
}

/// A future that resolves after a period of time.
pub struct Timer(oneshot::Receiver<()>);

impl Timer {
    /// Creates a new timer from a oneshot receiver.
