...
use crate::{
    AnyElement, App, AvailableSpace, Bounds, ContentMask, Element, ElementId, Entity, Hitbox,
    InteractiveElement, Interactivity, IntoElement, IsZero, LayoutId, ListSizingBehavior, Overflow,
...
impl Element for UniformList {
    ...
    fn prepaint(
        ...
        cx.with_window_app(|window, cx| {
            let style = self
                .interactivity
                .compute_style(global_id.as_ref(), None, window, cx);
                    ...
                    if self.item_count > 0 {
                        ...
                        window.with_content_mask(Some(content_mask), |window| {
                            for (mut item, ix) in items.into_iter().zip(visible_range.clone()) {
                                ...
                                item.layout_as_root_with_window(available_space, window, cx);
                                item.prepaint_at_with_window(item_origin, window, cx);
                            ...
                            for decoration in &self.decorations {
                                ...
                                decoration.layout_as_root_with_window(available_space, window, cx);
                                decoration.prepaint_at_with_window(bounds.origin, window, cx);
    ...
    fn paint(
        ...
        cx.with_window_app(|window, cx| {
            ...
            self.interactivity
                .paint(&mut interactivity_cx, hitbox.as_ref(), |_, window, cx| {
                    for item in &mut request_layout.items {
                        let mut item_cx = crate::PaintCx::new(window, cx, None, None, bounds);
                        item.paint(&mut item_cx);
                    }
                    for decoration in &mut request_layout.decorations {
                        let mut decoration_cx = crate::PaintCx::new(window, cx, None, None, bounds);
                        decoration.paint(&mut decoration_cx);
                    }
                })
        })
    }
...
impl UniformList {
    ...
    fn measure_item(
        ...
        item_to_measure.layout_as_root_with_window(available_space, window, cx)
    }
