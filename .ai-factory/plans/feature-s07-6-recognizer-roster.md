...
- **Blocked by:** S07.5b. This plan reads from the post-S07.5b surface: a single back-channel hook `set_arena_back_channel(pid, bc, idx)` (legacy single-pointer form deleted), `pressure: Option<PressureSample>` instead of `pressure: f32`, `AllowedButtonsFilter` available on every recogniser, transform-aware hit-test entries (`local_position`).
...
- [ ] **T7:** Implement `RecognizerLifecycle` for `MultiTapGestureRecognizer`:
  - `needs_back_channel() → true`.
  - `needs_arena_hold() → true` (each pointer's arena must hold; the dispatcher calls `arena.hold(pid)` per registration, schedules `multi_tap_window` release timer per pid).
  - `configure_settings(settings)` — copies `touch_slop` and `multi_tap_window` from `settings`.
  - `set_arena_back_channel(bc, _idx)` — stores `bc` into `arena_back_channel`. Index is single-shot, irrelevant for multi-pointer; we use the per-pointer hook instead. (Document this asymmetry in rustdoc — single-pointer accessors land first; multi-pointer accumulator follows.)
  - `set_arena_back_channel_for_pointer(pid, bc, idx)` — inserts `(pid, idx)` into `pointer_indexes`. Updates `arena_back_channel` if it was empty (idempotent across pointers).

- [ ] **T8:** Update `Window::dispatch_event` (or wherever the dispatcher schedules arena releases) to use `schedule_arena_release_with(pid, handle, cx, settings.multi_tap_window)` for recognisers whose `lifecycle().needs_arena_hold()` is true **and** `lifecycle().is_multi_pointer()` returns true. **Refinement:** rather than introduce another `is_multi_pointer` hook, simply key the timeout on whichever recogniser registered the hold. If two recognisers both want hold (DoubleTap + MultiTap on the same element), prefer the **shorter** timeout — the dispatcher computes `min(double_tap_timeout, multi_tap_window)` for the held arena and the longer-window recogniser will reject on its `multi_tap_window` later anyway. Update the timer-storage rustdoc.
...
- [ ] **T13:** Implement `GestureRecognizer` for `ForcePressGestureRecognizer`:
  - `add_pointer(pid, event)` — **rejects events with no real pressure** via `if event.pressure.is_none() { return; }`. After S07.5b, mouse-class events have `pressure: None`, so this is the canonical mouse-rejection path; there is no hard `kind` check. If `allowed_buttons_filter` is set, also check `filter(event.buttons, event.modifiers)` and bail on `false`. Otherwise check `event.buttons.contains(self.button)`. On pass: `state = Possible`, `pointer = Some(pid)`, `down_position = event.local_position`, `last_kind = event.kind`, `peak_triggered = false`.
  - `handle_event(event, window, cx)`:
    - All slop checks use `event.local_position` (post-S07.5b).
    - All pressure checks use `event.pressure.map(|p| p.normalised()).unwrap_or(0.0)` — a `None` mid-stream pressure is treated as zero (gesture should reject; can happen on weird platform-layer transitions).
...
- **Multi-pointer hold-timeout policy under DoubleTap + MultiTap on the same element.** Two recognisers both opting into `needs_arena_hold` is novel — picking `min(timeouts)` is the conservative default but means MultiTap always wins the timeout race when both compete. Net behavioural impact: DoubleTap on a MultiTap-equipped element gets less wall-clock to land its second tap.
  Mitigation: T8 documents this in rustdoc; T11 includes a unit test exercising both recognisers on the same element to lock the behaviour. If a downstream consumer flags it, alternative policies (per-recogniser timeout queues) can be considered in a follow-up — non-breaking.
