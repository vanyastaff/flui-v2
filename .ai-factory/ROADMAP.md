...
>
> Phase I (platform extraction) is FROZEN after S01 + S02a. **Active strategic work begins with Phase 0-K (Kernel Cleanup)** — fixing foundational issues in `flui-core` (broken Provider, no Widget Identity, `Render::&mut self` semantics, effect/frame contract, Element param explosion, action globals, coordinate-space type-safety, layout cache). K99, K15, K07, K05, K01, K02, K03, and K04 are all complete; the Phase 0-K critical chain is fully landed. **SF01 is also done as of 2026-05-12** — the Framework tier (Phase II-F) is now open and populated with its scaffolding (`flui-framework` crate, Widget / StatefulWidget / IntoWidget / WidgetState / Empty trait surface, `derive(Widget)` macro). SF02 (reconciliation) is the next Phase II-F item. Cross-cutting tracks (performance, architecture hygiene, testing infrastructure, release readiness) run continuously.
...
- [ ] **K06 Window decomposition + ownership split** — split `window.rs` (6123 lines, 222 pub methods) into `window/{lifecycle,layout,paint,hit_test,dispatch,focus,state,frame,actions}.rs`. Beyond cosmetic — split Window's monolithic borrow domain into `BuildOwner` / `PipelineOwner` / `SemanticsOwner` (Flutter-style independent owners). Closes E13 from `docs/promt.md`. **API-BREAKING** for any code depending on Window field access patterns. **Blocked-on: A10a** — A10a introduces transient `WindowCore(all 140 fields)` scaffold; K06 supersedes it with ownership-sharded `BuildOwner/PipelineOwner/SemanticsOwner` decomposition. K06 follow-up spec must explicitly redesign `WindowCore` boundaries. See `docs/superpowers/specs/2026-05-13-A10-xl-file-split-design.md` Risks #1.
...
- [ ] **K91 29 globs → explicit re-exports** — continuation of S01a.3 precedent. Closes E17 from `docs/promt.md` and roadmap A2. **SF01 cross-track contract (2026-05-12):** when replacing the `pub use element::*;` glob at `crates/flui-core/src/lib.rs:154`, the new explicit list MUST preserve crate-root visibility of `Key`, `ValueKey`, `GlobalKey` — `crates/flui-framework/src/key.rs` re-exports them at `flui_core::*` and will fail to compile if K91 strips them. Document this binding constraint in K91's eventual plan/spec.
...
- [x] **SF01 Widget + Key trait** — `Widget` / `StatefulWidget` / `WidgetState<W>` / `IntoWidget` / `Empty` traits + types in the new `flui-framework` Tier B crate, `Key`/`ValueKey`/`GlobalKey` re-exports of the K02 substrate, opt-in 7-item `prelude`, `#[derive(Widget)]` proc-macro with `#[widget(key)]` attribute in `flui-macros`, trybuild compile-pass/fail fixtures, `examples/widget_surface_demo` `cargo check`-only example. Amendment 1 (2026-05-12): `Widget::build` is required (not default) and `Empty` is the sealed null widget for trivial bodies — Rust 1.95 cannot resolve `impl IntoWidget` to `!`. Reviewer triple ran twice (pre-freeze T0.2 + post-impl T6.2). Plan: `.ai-factory/plans/feature-SF01-widget-key-trait.md`. Spec: `docs/superpowers/specs/2026-05-12-SF01-widget-key-trait-design.md`. Migration: `docs/superpowers/migrations/SF01-widget-key-trait.md`.
...
- [ ] **A10 XL-file decomposition (facade-pattern)** — split files >2500 LoC into facade + submodules per ADR-021. **A10a** `window.rs` (6036 → ~700 facade + 11 submodules, introduces `WindowCore` foundation pattern); **A10b** `geometry.rs` (3802 → ~150 facade + 8 per-type submodules); **A10c** `elements/div.rs` (3673 → ~200 facade + 5 submodules, preserving `Div`↔`Stateful` coupling). Spec: `docs/superpowers/specs/2026-05-13-A10-xl-file-split-design.md`. Policy: `docs/research/adr/ADR-021-xl-file-split-discipline.md`. **API-NEUTRAL** — public surface unchanged. Sequential (A10a → A10b → A10c). Closes 2 of ~29 globs from A2 as synergy (`pub use window::*`, `pub use elements::*`; `geometry::*` stays glob by ADR-021 Practice 4). Pre-PR triple-launch reviewers required for A10a PR 1.0 (`WindowCore` foundation) and PR 1.8 / 1.9 (K04 anchor + event dispatch).
- [ ] **A11+ XL-file decomposition (remaining)** — same facade-pattern applied to `app.rs` (3368), `platform.rs` (2419), `element.rs` (1902), `style.rs` (1548), `color.rs` (1164), `text_system.rs` deeper subsplit (1078), `key_dispatch.rs` (998). Each gets its own mini-spec under ADR-021 once A10 validates the pattern. May coordinate with K07 (AppCell — `AppCore`?), K01 (Provider — provider/ already split), K10 (Style decomposition — different concern but adjacent).
...
| SF01 Widget + Key trait (Tier B `flui-framework` crate skeleton; `Widget` / `StatefulWidget` / `WidgetState<W>` / `IntoWidget` / `Empty` traits + types; `Key`/`ValueKey`/`GlobalKey` re-exports; opt-in 7-item `prelude`; `#[derive(Widget)]` proc-macro with `#[widget(key)]` attribute; trybuild compile-pass/fail fixtures; `examples/widget_surface_demo` `cargo check`-only example; migration guide; tier-isolation QA note. Amendment 1: `Widget::build` required + sealed `Empty` null widget for trivial bodies — Rust 1.95 cannot resolve `impl IntoWidget` to `!`. Reviewer triple ran twice: pre-freeze (T0.2) on the design spec, post-impl (T6.2) on the landed code. K91 cross-track contract added: when K91 explicit-re-exports `flui_core::lib.rs:154`, preserve crate-root visibility of Key family. First Phase II-F spec; unblocks SF02/SF03/SF04 planning. Validation: `cargo test --workspace` pass (8 + 6 key/trait tests, 7 trybuild fixtures, 2 doctests, 3 macro unit tests); `cargo clippy --workspace --all-targets -- -D warnings` clean; `cargo fmt --check` clean. | 2026-05-12 |
