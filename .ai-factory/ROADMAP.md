...
>
> Phase I (platform extraction) is FROZEN after S01 + S02a. **Active strategic work begins with Phase 0-K (Kernel Cleanup)** — fixing foundational issues in `flui-core` (broken Provider, no Widget Identity, `Render::&mut self` semantics, effect/frame contract, Element param explosion, action globals, coordinate-space type-safety, layout cache). K99, K15, K07, and K05 are complete; the next critical-chain item is K01. The Framework tier (Phase II-F) and remaining Engine completeness (Phase II) are gated on Kernel Cleanup completion of the critical chain (K01 → K02 → K03 → K04). Cross-cutting tracks (performance, architecture hygiene, testing infrastructure, release readiness) run continuously.
...
- **K99 → K15 → K07 → K05 → K01 → K02 → K03 → K04**: Phase 0-K critical chain. Sequential. K99, K15, K07, and K05 are complete; K01 is next. Gates all SF specs.
