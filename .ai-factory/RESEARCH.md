...
**SF01 status (2026-05-12):** Widget + Key trait surface is complete — first Phase II-F spec landed on top of the fully-finished Phase 0-K kernel. New `flui-framework` Tier B crate at `crates/flui-framework/` exposes `Widget`, `StatefulWidget`, `WidgetState<W>` (forward-declared marker, body lands in SF04), `IntoWidget` with `impl<W: Widget> IntoWidget for W` blanket and sealed-against-dyn contract, `Empty` sealed null widget (per Amendment 1), and `Key`/`ValueKey`/`GlobalKey` re-exports from `flui_core`. Opt-in 7-item `prelude` excludes `WidgetState` for stability rationale. `#[derive(Widget)]` proc-macro in `flui-macros` with `#[widget(key)]` field attribute (field-name-agnostic, terminal-segment `Option<...Key>` type validation, struct-only). Trybuild fixtures (4 pass + 7 fail) lock the macro's compile-pass / compile-fail surface, covering all malformed `#[widget(...)]` meta forms (path, name-value, empty list, unknown sub-arg) plus duplicate-key, wrong key type, and enum-input rejection. `examples/widget_surface_demo/` proves the trait surface compiles for an out-of-tree consumer via `flui-framework` only. **Amendment 1 (2026-05-12)** during T3.1: `Widget::build` is required (not default `unimplemented!()`) and `Empty` is the sealed trivial-body return — Rust 1.95 cannot resolve `impl IntoWidget` to `!` and `feature(never_type)` is unstable. Reviewer triple ran twice (pre-freeze T0.2 on the spec, post-impl T6.2 on the landed code); 13 + 12 findings disposed, all blockers resolved. K91 cross-track contract recorded in ROADMAP.md K91 entry. ARCHITECTURE.md code examples updated to `impl IntoWidget`. Validation: `cargo test --workspace` pass (8 key roundtrip + 6 trait surface + 11 trybuild fixtures + 2 doctests + 3 macro unit tests), `cargo clippy --workspace --all-targets -- -D warnings` clean, `cargo fmt --check` clean. Phase II-F is now OPEN. Next item: SF02 (reconciliation algorithm). Spec: `docs/superpowers/specs/2026-05-12-SF01-widget-key-trait-design.md`. Plan: `.ai-factory/plans/feature-SF01-widget-key-trait.md`. Migration: `docs/superpowers/migrations/SF01-widget-key-trait.md`.

**K04 status (2026-05-12):** Effect / Frame contract is complete — the eighth and final Phase 0-K critical-chain spec. The App scheduler is now a typed seven-phase state machine (`PreFrame → AnimationTick → Build (reserved no-op for SF05) → Layout → Prepaint → Paint → PostFrame`) entered via `App::run_frame(window_id) -> FrameOutcome`. `FrameClock` samples the underlying `Clock` exactly once per frame (axiom P3) and serves every consumer the same `Instant`. `Effect::Defer { placement: DeferPlacement, callback }` replaces the placement-less variant; `App::defer(f)` preserves pre-K04 observable behavior by routing through `DeferPlacement::EndOfUpdate`. Per-phase drains use `FlushScope::Phase` (pre-body, full placement admission) and `FlushScope::PhasePost` (post-body, only `EndOfUpdate`) so matching-placement defers always carry one frame to their target phase. `AnimationController::value` caches per-frame keyed on `FrameClock::frame_index()` (monotonic) rather than `Instant` — two frames driven by a `TestClock` without `advance_clock` can share the same `Instant` but never share a frame index. `Window::on_pre_frame` (renamed from `on_next_frame`) and `Window::on_post_frame` (new) bookend each frame; App-level `App::on_pre_frame` / `App::on_post_frame` cover cross-window callbacks. `Window::request_animation_frame` is idempotent via `Cell<bool>`. Per-window `next_frame_callbacks` migrated from `Rc<RefCell<Vec<_>>>` to `RefCell<SmallVec<[_; 4]>>` directly on `Window`. `TestApp::advance_frame` is the canonical test-mode frame driver. Panic-in-phase recovery via `App::abort_frame_after_panic` keeps the App usable. The K15 contract is unchanged — joint K15+K04 paragraph published in the `flui_core::reentrancy` module docs. Validation: 417 `cargo test -p flui-core --lib --features test-support` pass; `cargo fmt --check` clean; cross-crate `cargo check` clean. The Phase 0-K critical chain is now FULLY COMPLETE. Phase II-F (SF03 BuildCx ergonomics, SF04 InheritedWidget facade, SF05 setState + dirty list) is unblocked and can begin planning. K04+1 follow-ups: flip `auto_advance_frames_on_flush` default to `false` once Tier-C tests migrate; remove the deprecated `Window::on_next_frame` alias; integrate the platform `on_request_frame` callback to call `App::run_frame` directly (today's production path stays inline). Spec: `docs/superpowers/specs/2026-05-11-K04-effect-frame-contract-design.md`. Migration: `docs/superpowers/migrations/K04-effect-frame-contract.md`.
...
1. **K99, K15, K07, K05, K01, K02, K03, K04, and SF01 are all complete.** Phase 0-K critical chain is closed; Phase II-F is open with its scaffolding (Widget / StatefulWidget / IntoWidget / WidgetState / Empty trait surface, `derive(Widget)` macro).
2. **Next run `/aif-plan full SF02-reconciliation`** — sibling matching by `(TypeId, Key)`, `did_update_widget` / `dispose` lifecycle hooks, position-fallback when keys absent, duplicate-key collision handling, O(siblings) per level invariant. Depends on SF01 (landed) and K15 (landed).
3. **SF03 / SF04 / SF05 / SF06 / SF07 / SF08** can be planned once SF02 lands.
4. **Hygiene K90-K98 in parallel slots** — small independent PRs, can land any time. K91 has a binding SF01 cross-track contract (preserve crate-root visibility of Key family).
5. **Internal-org K06, K08, K10, K11** — schedule them alongside Phase II-F work when they don't conflict.
